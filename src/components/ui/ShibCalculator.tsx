...
import { SHIB_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/shibData';

export default function ShibCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=shiba-inu&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.['shiba-inu']?.usd ?? null))
      .catch(() => {});
  }, []);
  ...
  const fmtShibPrice = (p: number) => {
    // Format tiny SHIB price: 0.0000110 → "$0.00001100"
    return `$${p.toFixed(7)}`;
  };
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил SHIB...' : 'What if I had bought SHIB...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена SHIB: ' : 'Current SHIB price: '}
        <span className="font-semibold text-foreground">
          {currentPrice ? fmtShibPrice(currentPrice) : '…'}
        </span>
      </p>
      ...
      <div className="grid grid-cols-1 gap-4 max-w-sm">
        {SHIB_INVESTMENT_REFERENCE.map(ref => {
          const shibBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? shibBought * currentPrice : null;
            ...
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-foreground">{fmtShibPrice(ref.price)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено SHIB' : 'SHIB received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? fmtShib(shibBought) : '—'} SHIB
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Историческая цена: 2022 г. — ~$0.0000110 (данные CoinMarketCap). SHIB запущен в августе 2020 г. Не является финансовой рекомендацией.'
