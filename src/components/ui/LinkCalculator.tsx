...
import { LINK_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/linkData';

export default function LinkCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=chainlink&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.chainlink?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил LINK...' : 'What if I had bought LINK...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена LINK: ' : 'Current LINK price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {LINK_INVESTMENT_REFERENCE.map(ref => {
          const linkBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? linkBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено LINK' : 'LINK received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? linkBought.toFixed(2) : '—'} LINK
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены: 2019 г. — ~$2.50, 2021 г. — ~$20.00 (данные CoinMarketCap). Не является финансовой рекомендацией.'
