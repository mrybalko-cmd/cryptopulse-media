...
import { DOT_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/dotData';

export default function DotCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=polkadot&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.polkadot?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил DOT...' : 'What if I had bought DOT...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена DOT: ' : 'Current DOT price: '}
      ...
      <div className="grid grid-cols-1 gap-4 max-w-sm">
        {DOT_INVESTMENT_REFERENCE.map(ref => {
          const dotBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? dotBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено DOT' : 'DOT received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? dotBought.toFixed(2) : '—'} DOT
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Историческая цена: 2021 г. — ~$15.00 (данные CoinMarketCap). Polkadot запущен в 2020 г. Не является финансовой рекомендацией.'
