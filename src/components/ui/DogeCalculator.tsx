...
import { DOGE_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/dogeData';

export default function DogeCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=dogecoin&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.dogecoin?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил DOGE...' : 'What if I had bought DOGE...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена DOGE: ' : 'Current DOGE price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {DOGE_INVESTMENT_REFERENCE.map(ref => {
          const dogeBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? dogeBought * currentPrice : null;
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-foreground">
                    ${ref.price < 0.01 ? ref.price.toFixed(5) : ref.price.toFixed(2)}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено DOGE' : 'DOGE received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? fmtTokens(dogeBought) : '—'} DOGE
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены: 2016 г. — ~$0.00012, 2021 г. — ~$0.22 (данные CoinMarketCap). Не является финансовой рекомендацией.'
