...
import { ADA_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/adaData';

export default function AdaCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=cardano&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.cardano?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил ADA...' : 'What if I had bought ADA...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена ADA: ' : 'Current ADA price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {ADA_INVESTMENT_REFERENCE.map(ref => {
          const adaBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? adaBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено ADA' : 'ADA received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? adaBought.toLocaleString('en-US', { maximumFractionDigits: 0 }) : '—'} ADA
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены: 2019 г. — ~$0.065, 2021 г. — ~$1.20 (данные CoinMarketCap). Не является финансовой рекомендацией.'
          : '* Approximate calculation. Historical prices: 2019 — ~$0.065, 2021 — ~$1.20 (CoinMarketCap data). Not financial advice.'}
      </p>
    </div>
  );
}
