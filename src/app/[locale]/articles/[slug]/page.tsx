// Час, а не пять минут. Страницы материалов — это 2200 из 2642 адресов в
// карте сайта, и при пятиминутном окне роботы пересобирали весь архив
// круглосуточно: 5,04 млн записей ISR за 23 дня, 83 пересборки на страницу в
// сутки, то есть каждые 17 минут. В сентябрьском счёте это $20,17 из $35,37.
//
// Свежесть текста от окна не зависит: публикация в админке сбрасывает
// персональный тег материала и его путь напрямую, правка видна сразу.
// Счётчик просмотров клиентский и тоже не замирает. Устаревать на час может
// только боковая колонка «Популярное» и выбранный баннер — обе вещи с
// собственным сбросом при изменении.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    ...(article.seo?.noIndex && { robots: { index: false, follow: true, googleBot: { index: false, follow: true } } }),
    alternates: {
...
export default async function ArticlePage({ params }: Props) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      <h1 className={`text-2xl sm:text-3xl font-bold text-foreground leading-tight mb-4${article.ownBadge ? ' own-mark-lead' : ''}`}>
        {article.title}
      </h1>
