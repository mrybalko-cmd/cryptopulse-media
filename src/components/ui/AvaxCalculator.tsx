...
import { AVAX_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/avaxData';

export default function AvaxCalculator({ locale }: { locale: string }) {
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил AVAX...' : 'What if I had bought AVAX...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена AVAX: ' : 'Current AVAX price: '}
      ...
      <div className="grid grid-cols-1 gap-4 max-w-sm">
        {AVAX_INVESTMENT_REFERENCE.map(ref => {
          const avaxBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? avaxBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено AVAX' : 'AVAX received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? avaxBought.toFixed(2) : '—'} AVAX
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Историческая цена: 2021 г. — ~$13.50 (данные CoinMarketCap). Avalanche запущен в сентябре 2020 г. Не является финансовой рекомендацией.'
