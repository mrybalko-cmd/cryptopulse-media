...
import { XRP_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/xrpData';

export default function XrpCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=ripple&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.ripple?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил XRP...' : 'What if I had bought XRP...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена XRP: ' : 'Current XRP price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {XRP_INVESTMENT_REFERENCE.map(ref => {
          const xrpBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? xrpBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="mb-3">
                <p className="text-xs text-muted">{isRu ? 'Получено XRP' : 'XRP received'}</p>
                <p className="text-sm font-semibold text-foreground">
                  {amount > 0
                    ? xrpBought >= 1000
                      ? xrpBought.toLocaleString('en-US', { maximumFractionDigits: 0 })
                      : xrpBought.toFixed(2)
                    : '—'} XRP
                </p>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены — средние значения за июль соответствующего года по данным CoinMarketCap. Не является финансовой рекомендацией.'
