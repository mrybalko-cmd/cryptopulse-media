...
import { BNB_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/bnbData';

export default function BnbCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=binancecoin&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.binancecoin?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил BNB...' : 'What if I had bought BNB...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена BNB: ' : 'Current BNB price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {BNB_INVESTMENT_REFERENCE.map(ref => {
          const bnbBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? bnbBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено BNB' : 'BNB received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? bnbBought.toFixed(2) : '—'} BNB
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены: 2019 г. — ~$25, 2021 г. — ~$310 (данные CoinMarketCap). Не является финансовой рекомендацией.'
