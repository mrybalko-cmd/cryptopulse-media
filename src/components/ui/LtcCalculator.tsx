...
import { LTC_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/ltcData';

export default function LtcCalculator({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    fetch('/api/price?ids=litecoin&vs_currencies=usd')
      .then(r => r.json())
      .then(d => setCurrentPrice(d?.litecoin?.usd ?? null))
      .catch(() => {});
  }, []);
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил Litecoin...' : 'What if I had bought Litecoin...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена LTC: ' : 'Current LTC price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {LTC_INVESTMENT_REFERENCE.map(ref => {
          const ltcBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? ltcBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено LTC' : 'LTC received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? ltcBought.toFixed(2) : '—'} LTC
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Исторические цены: 2016 г. — ~$4.00, 2021 г. — ~$130.00 (данные CoinMarketCap). Не является финансовой рекомендацией.'
