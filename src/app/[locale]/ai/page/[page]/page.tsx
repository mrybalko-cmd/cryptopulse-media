...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = isRu
    ? `Новости и аналитика об искусственном интеллекте в мире криптовалют и блокчейна — страница ${page}.`
    : `News and analysis on artificial intelligence in the crypto and blockchain world — page ${page}.`;
  return {
    title,
    description,
    openGraph: buildOg({ url: `${BASE}/${locale}/ai/page/${page}`, title, description, locale }),
    twitter: buildTwitter({ url: `${BASE}/${locale}/ai/page/${page}`, title, description, locale }),
    alternates: {
      canonical: `${BASE}/${locale}/ai/page/${page}`,
      languages: {
        ru: `${BASE}/ru/ai/page/${page}`,
        en: `${BASE}/en/ai/page/${page}`,
        'x-default': `${BASE}/en/ai/page/${page}`,
      },
    },
  };
}
