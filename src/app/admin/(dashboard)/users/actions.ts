...
export async function createUserAction(formData: FormData) {
  ...
  await createAdminUser({ name, email, passwordHash, isOwner, permissions: parsePermissions(formData) });
...
export async function updateUserAction(id: string, formData: FormData) {
  ...
  await updateAdminUser(id, { name, isOwner, active, permissions: parsePermissions(formData) });
