...
(async () => {
  ...
  await write.patch(doc._id).set({ page, hasPage: true, checkedAt: '2026-08-20' }).commit();
  console.log(`опубликовано: ${slug} (${FIELDS.filter(k => en[k]).length}/10 полей EN, ${FIELDS.filter(k => ru[k]).length}/10 RU)`);
})();
