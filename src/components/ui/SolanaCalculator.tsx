...
import { SOL_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/solanaData';

export default function SolanaCalculator({ locale }: { locale: string }) {
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил Solana...' : 'What if I had bought Solana...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена SOL: ' : 'Current SOL price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 gap-4 max-w-sm">
        {SOL_INVESTMENT_REFERENCE.map(ref => {
          const solBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? solBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                ...
                <div>
                  <p className="text-xs text-muted">{isRu ? 'Получено SOL' : 'SOL received'}</p>
                  <p className="text-sm font-semibold text-foreground">
                    {amount > 0 ? solBought.toFixed(2) : '—'} SOL
                  </p>
                </div>
              </div>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Историческая цена — среднее значение за июль 2021 г. по данным CoinMarketCap. Solana запущена в 2020 году. Не является финансовой рекомендацией.'
