...
import { BTC_INVESTMENT_REFERENCE, INVESTMENT_AMOUNTS } from '@/lib/bitcoinData';

export default function BitcoinCalculator({ locale }: { locale: string }) {
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">
          {isRu ? 'Что если бы я купил Bitcoin...' : 'What if I had bought Bitcoin...'}
        </h2>
      </div>
      <p className="text-sm text-muted mb-6">
        {isRu ? 'Текущая цена BTC: ' : 'Current BTC price: '}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {BTC_INVESTMENT_REFERENCE.map(ref => {
          const btcBought = amount > 0 ? amount / ref.price : 0;
          const currentValue = currentPrice && amount > 0 ? btcBought * currentPrice : null;
          ...
          return (
            <div key={ref.yearsAgo} className="bg-background border border-border rounded-xl p-4 flex flex-col">
              ...
              <div className="mb-3">
                <p className="text-xs text-muted">{isRu ? 'Получено BTC' : 'BTC received'}</p>
      ...
      <p className="text-xs text-muted mt-4 leading-relaxed">
        {isRu
          ? '* Расчёт приблизительный. Историческая цена — среднее значение за июль соответствующего года по данным CoinMarketCap. Не является финансовой рекомендацией.'
